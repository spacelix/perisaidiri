<div id="anggotaModalEdit-{{$anggota->id}}" tabindex="-1" aria-hidden="true"
                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="name" id="name"
                               ...
                               placeholder="Yoga Mulya Kusuma" value="{{old('name', $anggota->name)}}">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" name="no_anggota" id="no_anggota"
                               class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                               placeholder="100402843" value="{{old('email', $anggota->email)}}">
                        @error('no_anggota')
                        <span class="text-red-500 text-sm">
                            {{ $message }}
                        </span>
                    ...
                    <div>
                        ...
                        <select id="unit_id" name="unit_id"
                            ...
                            @foreach($units as $unit)
                                <option
                                    value="{{ $unit->id }}" {{ $unit->id == $anggota->unit_id ? 'selected' : '' }}>{{ $unit->name }}</option>
                            @endforeach
                        ...
                        <select id="tingkatan_id" name="tingkatan_id"
                            ...
                            @foreach($tingkatans as $tingkatan)
                                <option value="{{$tingkatan->id}}"
                                        @if($tingkatan->id == old('tingkatan_id', $anggota->tingkatan_id)) selected @endif>{{$tingkatan->nama}}</option>
                            @endforeach
                        </select>
                        @error('tingkatan_id')
                        <span class="text-red-500 text-sm">
                    ...
                    <div>
                        ...
                        <select id="jenis_kelamin" name="jenis_kelamin"
                            ...
                            <option value="L"
                                    @if('L' == old('jenis_kelamin', $anggota->jenis_kelamin)) selected @endif>Laki-laki
                            </option>
                            <option value="P"
                                    @if('P' == old('jenis_kelamin', $anggota->jenis_kelamin)) selected @endif>Perempuan
                            </option>
                        </select>
                        @error('jenis_kelamin')
                        <span class="text-red-500 text-sm">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <div class="flex gap-3 items-center">
                            <img id="imgPreview-edit"
                                 src="{{asset('storage/anggota/'.$anggota->foto)}}"
                                 class=" w-16 h-16 rounded-full" alt="Preview Foto"/>
                            <div class="w-full">
                                <input
            ...
            $('#foto').change(function () {
                ...
                if (file) {
                    ...
                    reader.onload = function (event) {
                        ...
                        $('#imgPreview-edit').attr('src', event.target.result);
                    }
